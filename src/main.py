...
def main(
    ...
    while ai_cctv.cap.isOpened():
        ...
        resized_frame = ai_cctv.resize_frame(frame)
        mask_resized = cv2.resize(mask, (resized_frame.shape[1], resized_frame.shape[0])) if mask is not None else None
        frame_duration = 1 / frame_rate
        frame, emp_boxes_info, act_boxes_info = ai_cctv.process_frame(resized_frame, mask_resized)
