...
class AICCTV:
    ...
    def process_frame(self, frame, conf_th, mask, color=(58, 73, 141)):
        ...
        def activity(frame, color=(0, 255, 0)):
            act_boxes_info = []
            results = self.act_model(source=frame_region.to(self.device), stream=True)
            for r in results:
                for box in r.boxes:
        ...
        results = self.model(source=frame_region.to(self.device), stream=True)
        boxes_info = []
        boxes = []
        confidences = []
        labels = []
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        while True:
            if not frame_queue.empty():
                ...
                # Process frame using a separate thread
                future = executor.submit(aicctv.process_frame, frame, list_conf[count], mask)
                frame, boxes_info, act_boxes_info = future.result()
