...
def draw_table(img, data, percentages, row_height=40):
    ...
    headers = ["Employee", "Wrapping", "Unloading", "Packing", "Sorting", "Idle", "Absent"]
...
def insert_data_to_mysql(cursor, cam, timestamp, emp_class, times):
    try:
        ...
        """
        cursor.execute(query, (cam, timestamp, emp_class, times["wrapping_time"], times["unloading_time"], times["packing_time"], times["sorting_time"], times["idle_time"], times["absent_time"]))
    except Error as e:
        print(f"Error: {e}")
...
def main(video_path, model_emp_path, model_act_path, emp_conf_th, act_conf_th, scale):
    ...
    try:
        connection = mysql.connector.connect(host="localhost", database="report_ai_cctv", user="robot", password="robot123", port=3307)
